# Graceful Shutdown (Kontrollü Kapanma)

## 1. Genel Bakış ve Tanım
SIGTERM/SIGINT sinyallerinde aktif HTTP isteklerini tamamlayıp, worker kuyruklarını ve DB bağlantılarını düzenli kapatma mekanizması.
...
Gelen İstek / Görev
        |
        v
[ graceful_shutdown Kontrol Katmanı ]
        |
...
- **İmplementasyon**: `graceful_shutdown.go`
- **Test Senaryoları**: `graceful_shutdown_test.go`
- **İngilizce Doküman**: `graceful_shutdown.md`
